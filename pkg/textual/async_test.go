...
func TestAsync_MapsValuesAndClosesOutput(t *testing.T) {
	...
	out := Async(ctx, in, func(_ context.Context, v int) int { return v * 2 })
...
func TestAsync_StopsOnContextCancellation(t *testing.T) {
	...
	in := make(chan int)
	out := Async(ctx, in, func(_ context.Context, v int) int { return v })
...
func TestAsync_RecoversPanicAndStoresInContextPanicStore(t *testing.T) {
	...
	out := Async(ctx, in, func(_ context.Context, v int) int {
		panic("boom")
	})
...
func TestAsyncEmitter_EmitsMultipleValuesAndClosesOutput(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()
	...
	out := AsyncEmitter(ctx, in, func(_ context.Context, v int, emit func(int)) {
		emit(v)
		emit(v * 10)
	})
...
func TestAsyncEmitter_StopsOnContextCancellation(t *testing.T) {
	// Use a cancellable context. Cancellation is the only way for AsyncEmitter to stop
	// here because we never close and never send on `in`.
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	...
	// Use a separate wait context so the collection doesn't abort just because
	// ctx has been canceled. We want to assert that AsyncEmitter closes its output.
	waitCtx, waitCancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer waitCancel()
...
func TestAsyncs_RecoversPanicAndStoresInContextPanicStore(t *testing.T) {
	baseCtx, baseCancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer baseCancel()
	...
	out := AsyncEmitter(ctx, in, func(_ context.Context, _ int, _ func(int)) {
		panic("boom")
	})
