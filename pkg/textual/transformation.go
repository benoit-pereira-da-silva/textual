...
type Transformation[P Processor] struct {
	...
	// Processor is the processing stage that turns an input Result into one
...
//     string (DecodeText).
//  2. A single Result is created from that text and sent into the underlying
//     Processor.
//  3. The input channel is then closed to signal "no more Results" to the
//     Processor.
//  4. All Results produced on the processor's output channel are encoded to
...
//   - On any encoding error, the derived context is canceled and the
//     remaining results are drained in a background goroutine to avoid
//     blocking downstream senders.
//   - r and w are always closed before Process returns (success or error),
//     via the ignoreErr helper.
//
// End‑of‑stream signalling:
//
//   - Process tells the Processor that no more input is coming by closing
//     the input channel after sending the single initial Result.
...
func (t Transformation[P]) Process(ctx context.Context, r io.ReadCloser, w io.WriteCloser) error {
	...
	// Step 4: consume every Result until the processor closes its output
	// channel. This is how we know "there are no more Result to come".
	for {
		select {
		case <-ctx.Done():
...
// EncodeResult encodes a single Result into the Transformation's destination
